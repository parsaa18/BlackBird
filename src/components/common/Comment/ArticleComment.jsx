...
import ContainerArticleComment from "./countainerArticleComment";
import { HiX } from "react-icons/hi";

const NestedCommentSystemArticle = ({ params }) => {
	...
	const CallApi = async () => {
		const res = await ArticleCommentAPI(newsID);
		setComments(res);
	};
	...
	return (
		<div className="md:p-8 flex justify-center flex-col rounded-3xl h-full">
				...
				{comments.length > 3 && (
					<div className="w-full flex items-center justify-center p-10">
						...
						>
							{viewMore ? (
								<>
									مشاهده کمتر <HiX className="text-2xl my-1" />
								</>
							) : (
								<>
									<img src={Plus} />
...
export default NestedCommentSystemArticle;
